...
export const scrapeC21Sunsets = async (config, filters) => {
    ...
    try {
        ...
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Navigation timeout")), 25000));
        try {
                ...
                return articles.map((article) => {
                    ...
                    const priceElement = allElements.find((el) => {
                        ...
                        return text.includes('US$') && text.match(/US\$\s*\d/) &&
                            !el.closest('button') &&
                            el.offsetParent !== null; // Verificar que sea visible
                    });
                    ...
                    try {
                        ...
                        const potentialBadgeElements = Array.from(article.querySelectorAll('span, div, p')).filter((el) => {
                            ...
                            return text.length > 0 && text.length < 30 && badgeKeywords.some(keyword => text.toLowerCase() === keyword || text.toLowerCase().includes(keyword));
                        });
        ...
        for (let pageNum = 1; pageNum < maxPages; pageNum++) {
            try {
                ...
                const showMoreButton = await page.evaluate(() => {
                    ...
                    const dataNextElement = Array.from(document.querySelectorAll('[data-next]')).find(el => el.offsetParent !== null);
                    if (dataNextElement) {
                        return {
                            found: true,
