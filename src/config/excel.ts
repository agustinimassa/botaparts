...
import { ExcelConfig, Filters, NotificationPrefs, SourceConfig } from "../models/types.js";
 ...
 */
function parseExcelWorkbook(workbook: ExcelJS.Workbook, excelPath: string): ExcelConfig {
  ...
  if (filtersSheet.rowCount > 1) {
      ...
      if (value !== null && value !== undefined) {
        ...
        if (key === "maxPriceUSD" || key === "minPriceUSD" || key === "minBeds" || key === "minBaths") {
          ...
          if (typeof value === "string") {
            (filters as any)[key] = value.split(",").map((v) => v.trim()).filter(Boolean);
          } else if (Array.isArray(value)) {
            (filters as any)[key] = value.map((v) => v.toString());
          } else {
            (filters as any)[key] = [value.toString()];
          }
    ...
    if (!filters.maxPriceUSD || isNaN(filters.maxPriceUSD as number)) {
      logger.warn({ 
      ...
      }, `💰 Precio máximo configurado: $${(filters.maxPriceUSD as number).toLocaleString()} USD`);
    }
  }
    ...
    headerRowNotif.eachCell({ includeEmpty: false }, (headerCell, colNumber) => {
      ...
      if (value !== null && value !== undefined) {
        if (key === "emails") {
          ...
          (notifications as any)[key] = value?.toString() || value;
        }
      }
    });
  }
  ...
  logger.info(
    { fuentes: sources.length, emails: notifications.emails?.length ?? 0 },
    "Config Excel cargada",
  );
 ...
 */
export const loadExcelConfig = async (excelPath?: string): Promise<ExcelConfig> => {
  ...
  const googleSheetUrl =
    process.env.GOOGLE_SHEET_URL || process.env.GOOGLE_SHEET_PUBLISHED_URL;
  
  if (!googleDriveFileId && !googleSheetUrl) {
    throw new Error(
      "❌ Se requiere configuración de Google Sheets/Drive.\n" +
  ...
  try {
    ...
    throw new Error(
      `No se pudo descargar el Excel desde Google Drive: ${error.message}\n` +
      "   Verifica que:\n" +
